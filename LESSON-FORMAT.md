...
- **When a primitive is Claude-Code-only, say so plainly and give the Cursor path.** Native subagents (`.claude/agents/`, `/agents`), file-defined skills (`.claude/skills/SKILL.md`), and `/`-slash commands are Claude Code features. In those lessons (16, 17) tell a Cursor student to run that one lesson in Claude Code (installed in Lesson 2), and name Cursor's nearest analog (custom modes, `.cursor/commands/`, `.cursor/rules/`). Rules files: CLAUDE.md (Claude Code) vs AGENTS.md / `.cursorrules` (Cursor) — always reference both. Autonomy modes: `Shift+Tab` in Claude Code vs the mode selector in Cursor.
